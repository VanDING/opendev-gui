...
impl SandboxBackend for LandlockBackend {
    ...
    fn apply(&self, cmd: &mut Command, request: &ExecRequest) -> Result<(), BackendError> {
        ...
        if !abi.is_supported() {
            return Err(BackendError::NotSupported("Landlock ABI v1 not supported".into()));
        }
        ...
        let mut ruleset = landlock::Ruleset::new()
            ...
            .create()
            .map_err(|e| {
                BackendError::ApplyFailed(format!("Landlock ruleset create failed: {}", e))
            })?;

        // Allow access to common system paths
        let common_paths =
            ["/usr", "/bin", "/lib", "/lib64", "/etc", "/dev", "/proc", "/sys", "/tmp", "/var/tmp"];

        for path in &common_paths {
            if let Ok(metadata) = std::fs::metadata(path) {
                if metadata.is_dir() {
                    let _ = ruleset
                        .add(landlock::path_beneath(path, landlock::AccessFs::from_all(abi)));
                }
            }
        }
        ...
        for path in &request.capabilities.read {
            let _ = ruleset.add(landlock::path_beneath(path, landlock::AccessFs::from_read(abi)));
        }

        // Allow requested write paths
        for path in &request.capabilities.write {
            let _ = ruleset.add(landlock::path_beneath(path, landlock::AccessFs::from_all(abi)));
        }

        // Allow CWD
        let _ =
            ruleset.add(landlock::path_beneath(&request.cwd, landlock::AccessFs::from_all(abi)));

        // Restrict the ruleset
        let status = ruleset.restrict_self().map_err(|e| {
            BackendError::ApplyFailed(format!("Landlock restrict_self failed: {}", e))
        })?;
