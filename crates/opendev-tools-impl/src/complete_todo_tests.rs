...
async fn test_complete_by_id() {
    ...
    let result = tool.execute(make_args(&[("id", serde_json::json!("1"))]), &ctx).await;
    assert!(result.success);
    assert_eq!(mgr.lock().unwrap().get(1).unwrap().status, TodoStatus::Completed);
}
...
async fn test_complete_all_message() {
    ...
    let ctx = ToolContext::new("/tmp");
    tool.execute(make_args(&[("id", serde_json::json!("1"))]), &ctx).await;
    let result = tool.execute(make_args(&[("id", serde_json::json!("2"))]), &ctx).await;
...
async fn test_complete_not_found() {
    let (tool, _mgr) = make_tool();
    let ctx = ToolContext::new("/tmp");
    let result = tool.execute(make_args(&[("id", serde_json::json!("999"))]), &ctx).await;
    assert!(!result.success);
}
...
async fn test_complete_by_numeric_value() {
    ...
    let result = tool.execute(make_args(&[("id", serde_json::json!(2))]), &ctx).await;
    assert!(result.success);
    assert_eq!(mgr.lock().unwrap().get(2).unwrap().status, TodoStatus::Completed);
}
