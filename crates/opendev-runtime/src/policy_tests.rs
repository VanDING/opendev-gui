...
fn test_disabled_allows_everything() {
    let config = ExecPolicyConfig::disabled();
...
fn test_writable_path_within_project() {
    ...
    assert!(config.check_writable_path(Path::new("/home/user/project/src/main.rs")).is_ok());
    assert!(config.check_writable_path(Path::new("/home/user/project")).is_ok());
    assert!(config.check_writable_path(Path::new("/home/user/other/file.rs")).is_err());
    assert!(config.check_writable_path(Path::new("/etc/passwd")).is_err());
}

#[test]
fn test_for_project_preset() {
    let config = ExecPolicyConfig::for_project(Path::new("/home/user/myapp"));
    assert!(config.enabled);
    assert!(config.check_command("cargo build").is_ok());
    assert!(config.check_command("git status").is_ok());
    assert!(config.check_command("rm -rf /").is_err());
    assert!(config.check_writable_path(Path::new("/home/user/myapp/src/lib.rs")).is_ok());
    assert!(config.check_writable_path(Path::new("/home/user/other/file")).is_err());
}
...
fn test_extract_base_command() {
    ...
    assert_eq!(extract_base_command("FOO=1 BAR=2 /usr/local/bin/node script.js"), "node");
...
fn test_multiple_writable_paths() {
    ...
    let config = ExecPolicyConfig {
        enabled: true,
        allowed_commands: vec![],
        writable_paths: vec!["/home/user/project".into(), tmp_path.to_string_lossy().to_string()],
    };
    assert!(config.check_writable_path(Path::new("/home/user/project/src/main.rs")).is_ok());
    ...
    assert!(config.check_writable_path(Path::new("/var/log/syslog")).is_err());
}
...
fn test_error_messages_are_descriptive() {
    ...
    let path_err = config.check_writable_path(Path::new("/etc/shadow")).unwrap_err();
...
fn test_path_traversal_blocked() {
    ...
    assert!(config.check_writable_path(Path::new("/home/user/project/../../etc/passwd")).is_err());
}
