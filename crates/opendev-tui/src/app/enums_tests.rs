...
fn test_reasoning_level_from_str() {
    ...
    assert_eq!(ReasoningLevel::from_str_loose("medium"), ReasoningLevel::Medium);
...
fn test_reasoning_level_to_config() {
    ...
    assert_eq!(ReasoningLevel::Low.to_config_string(), Some("low".to_string()));
    assert_eq!(ReasoningLevel::Medium.to_config_string(), Some("medium".to_string()));
}
