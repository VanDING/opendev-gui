...
fn test_paths_project_dir() {
    ...
    assert_eq!(paths.project_dir(), PathBuf::from("/tmp/test-project/.opendev"));
}
...
fn test_project_context_file() {
    ...
    assert_eq!(paths.project_context_file(), PathBuf::from("/tmp/myproject/AGENTS.md"));
}

#[test]
fn test_project_mcp_config() {
    let paths = Paths::new(Some(PathBuf::from("/tmp/myproject")));
    assert_eq!(paths.project_mcp_config(), PathBuf::from("/tmp/myproject/.mcp.json"));
}
...
fn test_opendev_dir_env_override() {
    ...
    assert_eq!(paths.global_settings(), PathBuf::from("/tmp/custom-opendev/settings.json"));
    assert_eq!(paths.global_sessions_dir(), PathBuf::from("/tmp/custom-opendev/sessions"));
    assert_eq!(paths.global_logs_dir(), PathBuf::from("/tmp/custom-opendev/logs"));
...
fn test_config_vs_data_separation() {
    ...
    assert!(paths.global_sessions_dir().starts_with("/tmp/override-opendev"));
