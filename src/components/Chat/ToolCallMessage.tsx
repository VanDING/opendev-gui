...
export function ToolCallMessage({ message, hasResult }: ToolCallMessageExtProps) {
  ...
  if (message.role === 'tool_call') {
    ...
    return (
      <div className={`bg-surface-elevated border border-border-default/15 rounded-lg px-4 py-3 ${
        hasResult === false ? 'tool-executing' : hasResult ? 'border-l-[3px] border-l-intent-success/50' : ''
      }`}>
        {/* Tool action header */}
        <div className="flex items-center gap-2 mb-2">
          <span className="text-content-tertiary font-mono text-sm leading-6 flex-shrink-0">▶</span>
          <span className="font-medium text-content-primary text-sm leading-6">
            {verb}
          </span>
          {summary && (
            <span className="text-content-secondary text-sm bg-surface-primary px-2 py-1 rounded border border-border-default/20 font-mono leading-6">
              {summary}
            </span>
          )}
        </div>
        ...
        {hasExpandableContent && (
          <button
            ...
            className="ml-4 text-sm text-content-tertiary hover:text-content-secondary font-medium mt-2 leading-6"
          >
        ...
        {!isExpanded && (toolName === 'run_command' || toolName === 'bash_execute') && fullOutput && fullOutput.split('\n').length > 4 && (
          <div className="ml-4 pl-3 border-l-2 border-border-default/30 mt-1">
        ...
        {hasExpandableContent && (
          ...
          >
            <div ref={expandRef} className="ml-4 mt-3 pl-3 border-t border-border-default/15 pt-3">
              {fullOutput && (toolName === 'edit_file' || toolName === 'apply_patch') && fullOutput.includes('@@') ? (
                <DiffViewer diff={fullOutput} />
              ) : fullOutput ? (
                <pre className="text-sm text-content-secondary font-mono bg-surface-primary border border-border-default/15 rounded p-3 overflow-x-auto leading-6">
                  {fullOutput}
                </pre>
              ) : null}
            </div>
